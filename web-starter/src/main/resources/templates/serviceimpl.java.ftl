...
<#if create??>
    ...
    @OpLog(type = OtherOpLogType.class, name = "新增${classDescription}，ID：{}", params = ${r'{"#'}${create.keys[0].name}${r'"}'})
...
<#if update??>
    ...
    @OpLog(type = OtherOpLogType.class, name = "修改${classDescription}，ID：{}", params = ${r'{"#'}${update.keys[0].name}${r'"}'})
    @Transactional(rollbackFor = Exception.class)
    @Override
    ...
    @OpLog(type = OtherOpLogType.class, name = "删除${classDescription}，ID：{}", params = ${r'{"#'}${keys[0].name}${r'"}'})
    @Transactional(rollbackFor = Exception.class)
    @Override
