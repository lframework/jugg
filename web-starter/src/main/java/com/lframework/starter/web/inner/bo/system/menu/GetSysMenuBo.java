package com.lframework.starter.web.inner.bo.system.menu;

import com.lframework.starter.common.utils.StringUtil;
import com.lframework.starter.web.core.bo.BaseBo;
import com.lframework.starter.web.core.utils.ApplicationUtil;
import com.lframework.starter.web.inner.entity.SysMenu;
import com.lframework.starter.web.inner.service.system.SysMenuService;
import io.swagger.v3.oas.annotations.media.Schema;
import lombok.Data;

@Data
public class GetSysMenuBo extends BaseBo<SysMenu> {

  private static final long serialVersionUID = 1L;

  /**
   * ID
   */
  @Schema(description = "ID")
  private String id;

  /**
   * 编号
   */
  @Schema(description = "编号")
  private String code;

  /**
   * 名称（前端使用）
   */
  @Schema(description = "名称（前端使用）")
  private String name;

  /**
   * 标题
   */
  @Schema(description = "标题")
  private String title;

  /**
   * 图标
   */
  @Schema(description = "图标")
  private String icon;

  /**
   * 组件类型
   */
  @Schema(description = "组件类型（前端使用）")
  private Integer componentType;

  /**
   * 组件（前端使用）
   */
  @Schema(description = "组件（前端使用）")
  private String component;

  /**
   * 自定义列表ID
   */
  @Schema(description = "自定义列表ID")
  private String customListId;

  /**
   * 自定义列表名称
   */
  @Schema(description = "自定义列表名称")
  private String customListName;

  /**
   * 自定义表单ID
   */
  @Schema(description = "自定义表单ID")
  private String customFormId;

  /**
   * 自定义表单名称
   */
  @Schema(description = "自定义表单名称")
  private String customFormName;

  /**
   * 自定义请求参数
   */
  @Schema(description = "自定义请求参数")
  private String requestParam;

  /**
   * 自定义页面ID
   */
  @Schema(description = "自定义页面ID")
  private String customPageId;

  /**
   * 父级ID
   */
  @Schema(description = "父级ID")
  private String parentId;

  /**
   * 父级名称
   */
  @Schema(description = "父级名称")
  private String parentName;

  /**
   * 路由路径（前端使用）
   */
  @Schema(description = "路由路径（前端使用）")
  private String path;

  /**
   * 是否缓存（前端使用）
   */
  @Schema(description = "是否缓存（前端使用）")
  private Boolean noCache;

  /**
   * 类型 0-目录 1-菜单 2-功能
   */
  @Schema(description = "类型 0-目录 1-菜单 2-功能")
  private Integer display;

  /**
   * 是否隐藏（前端使用）
   */
  @Schema(description = "是否隐藏（前端使用）")
  private Boolean hidden;

  /**
   * 权限
   */
  @Schema(description = "权限")
  private String permission;

  /**
   * 是否特殊菜单
   */
  @Schema(description = "是否特殊菜单")
  private Boolean isSpecial;

  /**
   * 状态
   */
  @Schema(description = "状态")
  private Boolean available;

  /**
   * 备注
   */
  @Schema(description = "备注")
  private String description;

  public GetSysMenuBo() {

  }

  public GetSysMenuBo(SysMenu dto) {

    super(dto);
  }

  @Override
  @SuppressWarnings("unchecked")
  public <A> BaseBo<SysMenu> convert(SysMenu dto) {

    return super.convert(dto, GetSysMenuBo::getDisplay);
  }

  @Override
  protected void afterInit(SysMenu dto) {

    this.display = dto.getDisplay().getCode();
    if (!StringUtil.isBlank(dto.getParentId())) {
      SysMenuService sysMenuService = ApplicationUtil.getBean(SysMenuService.class);
      this.parentName = sysMenuService.findById(dto.getParentId()).getTitle();
    }

    this.componentType = dto.getComponentType() == null ? null : dto.getComponentType().getCode();
  }
}
