...
public class CustomListBuilder {
    ...
    public CustomListConfig buildConfig(String id) {
        ...
        List<GenCustomListQueryParams> genCustomListQueryParamsList = genCustomListQueryParamsService.getByCustomListId(
                data.getId());
        List<GenCustomListDetail> genCustomListDetailList = genCustomListDetailService.getByCustomListId(
                data.getId());
        ...
        if (CollectionUtil.isNotEmpty(genCustomListQueryParamsList)) {
            for (GenCustomListQueryParams genCustomListQueryParams : genCustomListQueryParamsList) {
                ...
                queryParam.setDefaultValue(Convert.convert(entityDetail.getDataType().getClazz(), genCustomListQueryParams.getDefaultValue()));
                queryParam.setHasAvailableTag(
                        entityDetail.getViewType() == GenViewType.SELECT && entityDetail.getDataType() == GenDataType.BOOLEAN
        ...
        if (CollectionUtil.isNotEmpty(genCustomListDetailList)) {
            for (GenCustomListDetail genCustomListDetail : genCustomListDetailList) {
                ...
                if (genCustomListDetail.getType() == GenCustomListDetailType.MAIN_TABLE || genCustomListDetail.getType() == GenCustomListDetailType.SUB_TALBE) {
    ...
    private DataObjectQueryParamObj buildQueryParamObj(DataObjectQueryParamObj queryParamObj, CustomListConfig config) {
        ...
        if (CollectionUtil.isNotEmpty(queryParamObj.getConditions())) {
            for (DataObjectQueryParamObj.Condition condition : queryParamObj.getConditions()) {
                FieldConfig fieldConfig = config.getListConfig().getFields().stream().filter(t -> (condition.getTableAlias() + "_" + condition.getColumnName()).equals(t.getColumnName())).findFirst().orElse(null);
                if (fieldConfig != null) {
                    if (condition.getValue() != null) {
                        condition.setValue(Convert.convert(EnumUtil.getByCode(GenDataType.class, fieldConfig.getDataType()).getClazz(), condition.getValue()));
                    } else if (CollectionUtil.isNotEmpty(condition.getValues())) {
                        condition.setValues(condition.getValues().stream().map(t -> Convert.convert(EnumUtil.getByCode(GenDataType.class, fieldConfig.getDataType()).getClazz(), t)).collect(Collectors.toList()));
                    }
                }
