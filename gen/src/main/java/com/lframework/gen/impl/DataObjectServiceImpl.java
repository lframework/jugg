...
import com.lframework.gen.dto.dataobj.*;
...
public class DataObjectServiceImpl implements IDataObjectService {
    ...
    public void update(@NonNull UpdateDataObjectVo vo) {
        ...
        data.setDescription(StringUtil.isNotBlank(vo.getDescription()) ? vo.getDescription() : StringPool.EMPTY_STR);
    ...
    public void delete(@NonNull String id) {
        ...
        List<String> columnIds = columns.stream().map(GenDataObjectColumnDto::getId).collect(Collectors.toList());
    ...
    public void batchEnable(List<String> ids, String userId) {
        ...
        Wrapper<GenDataObject> wrapper = Wrappers.lambdaUpdate(GenDataObject.class).set(GenDataObject::getAvailable, Boolean.TRUE).set(GenDataObject::getUpdateBy, userId).in(GenDataObject::getId, ids);
    ...
    public void batchUnable(List<String> ids, String userId) {
        ...
        Wrapper<GenDataObject> wrapper = Wrappers.lambdaUpdate(GenDataObject.class).set(GenDataObject::getAvailable, Boolean.FALSE).set(GenDataObject::getUpdateBy, userId).in(GenDataObject::getId, ids);
        genDataObjectMapper.update(wrapper);
    }
    ...
    public DataObjectGenerateDto getGenerateById(String id) {
        ...
        List<GenCreateColumnConfigDto> createColumnConfigDtos = genCreateColumnConfigService.getByDataObjId(id);
        result.setCreateConfigs(createColumnConfigDtos);

        List<GenUpdateColumnConfigDto> updateColumnConfigDtos = genUpdateColumnConfigService.getByDataObjId(id);
        result.setUpdateConfigs(updateColumnConfigDtos);

        List<GenQueryColumnConfigDto> queryColumnConfigDtos = genQueryColumnConfigService.getByDataObjId(id);
        result.setQueryConfigs(queryColumnConfigDtos);

        List<GenQueryParamsColumnConfigDto> queryParamsColumnConfigDtos = genQueryParamsColumnConfigService.getByDataObjId(id);
        result.setQueryParamsConfigs(queryParamsColumnConfigDtos);

        List<GenDetailColumnConfigDto> detailColumnConfigDtos = genDetailColumnConfigService.getByDataObjId(id);
    ...
    public void setStatus(String id, DataObjectGenStatus status) {
        ...
        switch (status) {
            case CREATED: {
                Wrapper<GenDataObject> updateWrapper = Wrappers.lambdaUpdate(GenDataObject.class).set(GenDataObject::getGenStatus, status).eq(GenDataObject::getId, id).eq(GenDataObject::getGenStatus, DataObjectGenStatus.CREATED);
            ...
            case SET_GEN: {
                Wrapper<GenDataObject> updateWrapper = Wrappers.lambdaUpdate(GenDataObject.class).set(GenDataObject::getGenStatus, status).eq(GenDataObject::getId, id).in(GenDataObject::getGenStatus, DataObjectGenStatus.SET_TABLE, DataObjectGenStatus.SET_GEN);
                if (genDataObjectMapper.update(updateWrapper) != 1) {
